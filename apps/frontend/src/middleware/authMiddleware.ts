...
const authMiddleware: Middleware = (store: MiddlewareAPI<AppDispatch, RootState>) => (next) => async (action) => {
    if ((action as PayloadAction).type === 'auth/getUser') {
        try {
            const getUser = store.dispatch(authApiSlice.endpoints.getUser.initiate())
